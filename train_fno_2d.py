...
def train_epoch(
    ...
    with tqdm(train_loader, disable=not accelerator.is_local_main_process) as pbar:
        for batch_idx, (data, grid, target) in enumerate(pbar):
            ...
            output = model(data.view(B, H, W, -1), grid)
            ...
            pbar.set_description(f"Epoch {epoch} Train Loss: {loss.item():.6f}")
...
#     with torch.no_grad():
#         for data, grid, target in test_loader:
#             B, H, W, T, C = data.shape
#             output = model(data.view(B, H, W, -1), grid)
#             loss = F.mse_loss(output.view(B, H, W, -1), target)
#             total_loss += loss.item()
...
#                 l2_diff = torch.norm(pred - true)
#                 l2_true = torch.norm(true)
    ...
    with torch.no_grad():
        for data, grid, target in test_loader:
            ...
            for i, var in enumerate(['Vx', 'Vy', 'density', 'pressure']):
                ...
                # For density and pressure, we need to reverse the log1p transform if it was applied
                # if var in ['density', 'pressure']:
                #     true_denorm = torch.sinh(true_denorm)  # inverse of log1p(abs(x))*sign(x)
                #     pred_denorm = torch.sinh(pred_denorm)

                # Calculate L2RE with denormalized values
...
def main(
    ...
    model = FNO2d(num_channels=4, modes1=modes, modes2=modes, width=width, initial_step=20)
    ...
    for epoch in range(num_epochs):
        train_loss = train_epoch(model, train_loader, optimizer, scheduler, accelerator, epoch)
        test_loss, l2re_metrics = evaluate(model, test_loader, accelerator)

        # Log metrics
        if accelerator.is_local_main_process:
            if task is not None:
                task.logger.report_scalar("Loss", "Train", train_loss, epoch)
                task.logger.report_scalar("Loss", "Test", test_loss, epoch)
